Template.serverDemo.rendered = ->
  $('#container_serverDemo').highcharts
    chart:
      ...
      events: load: ->
        ...
        ren.path([
          ...
          dashstyle: 'dash').add()
        # Headers
        ren.label('Web client', 20, 40).css(fontWeight: 'bold').add()
        ren.label('Web service / CLI', 220, 40).css(fontWeight: 'bold').add()
        ren.label('Command line client', 440, 40).css(fontWeight: 'bold').add()
        # SaaS client label
        ren.label('SaaS client<br/>(browser or<br/>script)', 10, 82).attr(
          ...
          'stroke-width': 2
          padding: 5
          r: 5).css(color: 'white').add().shadow true
        # Arrow from SaaS client to Phantom JS
        ren.path(rightArrow).attr(
          'stroke-width': 2
          stroke: colors[3]).translate(95, 95).add()
        ...
        ren.label('Browser<br/>running<br/>Highcharts', 10, 180).attr(
          ...
          r: 5).css(
          color: 'white'
          width: '100px').add().shadow true
        # Arrow from Browser to Batik
        ren.path(rightArrow).attr(
          'stroke-width': 2
          stroke: colors[1]).translate(95, 205).add()
        ...
        ren.path(leftArrow).attr(
          'stroke-width': 2
          stroke: colors[1]).translate(95, 215).add()
        ...
        ren.path(leftArrow).attr(
          'stroke-width': 2
          stroke: colors[2]).translate(330, 90).add()
        ...
        ren.path(rightArrow).attr(
          'stroke-width': 2
          stroke: colors[2]).translate(330, 100).add()
