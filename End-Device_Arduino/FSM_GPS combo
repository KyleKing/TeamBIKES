...
SoftwareSerial XbeeSerial(3,4); //FIX THIS TO CORRECT PINS!!!!!!!!!!!!
...
void loop(void) {
  ...
  if (currState == READ) {
    ...
    /*  if (!flag)
      ...
      string_out +="0";
      string_out +=",";
      string_out += String(BIKE_ID);
      Serial.println(string_out);
      XbeeSerial.println(string_out);
     
    }
    while(1)
    { //Serial.println("Waiting for Xbee"); 
      if(XbeeSerial.available())
        {  //Serial.println("Inside XbeeSerial check");
          
           approval=XbeeSerial.read();
            if(approval=='y')
             {   currState=OPEN;
